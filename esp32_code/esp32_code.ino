/*
  Havo Sifati Monitoringi — ESP32 Arduino kodi
  Diplom ishi: "Havo sifatining bashoratli monitoringi uchun aqlli qurilma"

  Ulanish:
    MQ-135 DO → GPIO 23
    MQ-2   DO → GPIO 5
    MQ-7   DO → GPIO 19
    DHT22     → GPIO 4

  Ishlash tartibi:
    1. Wi-Fi ga ulanadi
    2. Har 30 sekundda sensorlarni o'qiydi
    3. HTTP POST orqali serverga yuboradi
*/

// ─────────────────────────────────────────────
// 1) #INCLUDE LAR
// ─────────────────────────────────────────────
#include <WiFi.h>
#include <HTTPClient.h>
#include <ArduinoJson.h>
#include <DHT.h>

// ─────────────────────────────────────────────
// 2) #DEFINE LAR
// ─────────────────────────────────────────────
#define DHT_PIN  4       // DHT22 data pini
#define DHT_TYPE DHT22   // DHT sensor turi

// ─────────────────────────────────────────────
// 3) KONSTANTALAR
// ─────────────────────────────────────────────

// Wi-Fi ma'lumotlari
const char* WIFI_SSID  = "esp32";
const char* WIFI_PAROL = "12123434";

// Server manzili
const char* SERVER_URL = "http://192.168.100.136:8000/api/sensor";

// Qurilma identifikatori
const char* DEVICE_ID  = "esp32_001";

// GPIO pinlari
const int MQ135_PIN = 23;
const int MQ2_PIN   = 5;
const int MQ7_PIN   = 19;

// Vaqt sozlamalari
const unsigned long YUBORISH_INTERVAL = 30000;  // 30 sekund (ms)

// ─────────────────────────────────────────────
// 4) STRUCT E'LONI
// ─────────────────────────────────────────────
struct SensorQiymat {
  int   mq135;     // 1=toza, 0=gaz aniqlandi
  int   mq2;       // 1=toza, 0=gaz aniqlandi
  int   mq7;       // 1=toza, 0=CO aniqlandi
  float harorat;   // °C (NAN = o'qilmadi)
  float namlik;    // %  (NAN = o'qilmadi)
};

// ─────────────────────────────────────────────
// 5) OBYEKT VA GLOBAL O'ZGARUVCHILAR
// ─────────────────────────────────────────────
DHT dht(DHT_PIN, DHT_TYPE);

unsigned long oxirgi_yuborish = 0;
int yuborish_soni = 0;

// ─────────────────────────────────────────────
// 6) FUNKSIYA PROTOTIPLARI
// ─────────────────────────────────────────────
void          wifiGaUlan();
SensorQiymat  sensorlarniOqi();
bool          serveraYubor(SensorQiymat q);
void          holatChiqar(SensorQiymat q);

// ─────────────────────────────────────────────
// 7) SETUP — bir marta ishga tushadi
// ─────────────────────────────────────────────
void setup() {
  Serial.begin(115200);
  delay(500);

  Serial.println("\n╔══════════════════════════════════════╗");
  Serial.println("║  Havo Sifati Monitoringi — ESP32    ║");
  Serial.println("║  Diplom loyihasi, 2025              ║");
  Serial.println("╚══════════════════════════════════════╝");

  // GPIO pinlarini sozlash
  pinMode(MQ135_PIN, INPUT);
  pinMode(MQ2_PIN,   INPUT);
  pinMode(MQ7_PIN,   INPUT);
  dht.begin();

  Serial.println("⚙ Sensor pinlari sozlandi:");
  Serial.print("  MQ-135 → GPIO "); Serial.println(MQ135_PIN);
  Serial.print("  MQ-2   → GPIO "); Serial.println(MQ2_PIN);
  Serial.print("  MQ-7   → GPIO "); Serial.println(MQ7_PIN);
  Serial.print("  DHT22  → GPIO "); Serial.println(DHT_PIN);

  // Wi-Fi ga ulanish
  wifiGaUlan();

  // Birinchi o'lchov darhol yuborilsin
  oxirgi_yuborish = millis() - YUBORISH_INTERVAL;

  Serial.print("⏱ Yuborish intervali: ");
  Serial.print(YUBORISH_INTERVAL / 1000);
  Serial.println(" sekund");
  Serial.println("✨ Qurilma ishga tushdi!\n");
}

// ─────────────────────────────────────────────
// 8) LOOP — doimo takrorlanadi
// ─────────────────────────────────────────────
void loop() {
  unsigned long hozir = millis();

  // Belgilangan vaqt o'tganda ma'lumot yuborish
  if (hozir - oxirgi_yuborish >= YUBORISH_INTERVAL) {
    oxirgi_yuborish = hozir;

    // Sensorlarni o'qish
    SensorQiymat q = sensorlarniOqi();

    // Serial Monitor ga chiqarish
    holatChiqar(q);

    // Serverga yuborish
    bool muvaffaqiyat = serveraYubor(q);
    if (!muvaffaqiyat) {
      Serial.println("⚠ Keyingi urinishda qayta yuboriladi.");
    }

    Serial.print("⏳ Keyingi o'lchov: ");
    Serial.print(YUBORISH_INTERVAL / 1000);
    Serial.println(" sekunddan so'ng\n");
  }

  // Wi-Fi uzilishini kuzatish
  if (WiFi.status() != WL_CONNECTED) {
    Serial.println("⚠ Wi-Fi uzildi! Qayta ulanilmoqda...");
    wifiGaUlan();
  }

  delay(100);  // protsessor yukini kamaytirish
}

// ─────────────────────────────────────────────
// 9) FUNKSIYALAR
// ─────────────────────────────────────────────

// Wi-Fi ga ulanish
void wifiGaUlan() {
  Serial.print("📡 Wi-Fi ga ulanilmoqda: ");
  Serial.println(WIFI_SSID);

  WiFi.mode(WIFI_STA);
  WiFi.begin(WIFI_SSID, WIFI_PAROL);

  int urinish = 0;
  while (WiFi.status() != WL_CONNECTED) {
    delay(500);
    Serial.print(".");
    urinish++;
    if (urinish > 40) {
      Serial.println("\n⚠ Ulanib bo'lmadi! Qayta urinilmoqda...");
      WiFi.disconnect();
      delay(1000);
      WiFi.begin(WIFI_SSID, WIFI_PAROL);
      urinish = 0;
    }
  }

  Serial.println("\n✅ Wi-Fi ga ulandi!");
  Serial.print("   IP manzili: ");  Serial.println(WiFi.localIP());
  Serial.print("   Signal:     ");  Serial.print(WiFi.RSSI()); Serial.println(" dBm");
}

// Sensorlarni o'qish
SensorQiymat sensorlarniOqi() {
  SensorQiymat q;

  // MQ sensor DO (Digital Output):
  //   HIGH (1) → toza havo  |  LOW (0) → gaz aniqlandi
  q.mq135 = digitalRead(MQ135_PIN);
  q.mq2   = digitalRead(MQ2_PIN);
  q.mq7   = digitalRead(MQ7_PIN);

  // DHT22 — harorat va namlik
  q.harorat = dht.readTemperature();
  q.namlik  = dht.readHumidity();

  if (isnan(q.harorat) || isnan(q.namlik)) {
    Serial.println("⚠ DHT22 o'qishda xato!");
    q.harorat = NAN;
    q.namlik  = NAN;
  }

  return q;
}

// Serverga HTTP POST yuborish
bool serveraYubor(SensorQiymat q) {
  if (WiFi.status() != WL_CONNECTED) {
    Serial.println("⚠ Wi-Fi yo'q! Yuborilmadi.");
    return false;
  }

  // JSON tuzish
  StaticJsonDocument<256> doc;
  doc["device_id"] = DEVICE_ID;
  doc["mq135"]     = q.mq135;
  doc["mq2"]       = q.mq2;
  doc["mq7"]       = q.mq7;

  if (!isnan(q.harorat)) doc["harorat"] = q.harorat; else doc["harorat"] = nullptr;
  if (!isnan(q.namlik))  doc["namlik"]  = q.namlik;  else doc["namlik"]  = nullptr;

  // PM va bosim — hozir ulanmagan
  doc["pm25"]  = nullptr;
  doc["pm10"]  = nullptr;
  doc["bosim"] = nullptr;

  String json;
  serializeJson(doc, json);

  Serial.print("📤 Yuborilmoqda: ");
  Serial.println(json);

  HTTPClient http;
  http.begin(SERVER_URL);
  http.addHeader("Content-Type", "application/json");
  http.setTimeout(10000);

  int httpKod = http.POST(json);

  if (httpKod == HTTP_CODE_OK || httpKod == HTTP_CODE_CREATED) {
    String javob = http.getString();
    Serial.print("✅ Server javobi (");
    Serial.print(httpKod);
    Serial.print("): ");
    Serial.println(javob);
    http.end();
    yuborish_soni++;
    return true;
  } else if (httpKod > 0) {
    Serial.print("⚠ HTTP xato kodi: ");
    Serial.println(httpKod);
  } else {
    Serial.print("❌ Ulanish xatosi: ");
    Serial.println(http.errorToString(httpKod));
  }

  http.end();
  return false;
}

// Sensor holatini Serial Monitor ga chiqarish
void holatChiqar(SensorQiymat q) {
  Serial.println("─────────────────────────────────────");
  Serial.print("🏭 MQ-135 (CO2/NH3/Benzol): ");
  Serial.println(q.mq135 == 1 ? "✅ TOZA" : "🚨 GAZ ANIQLANDI!");

  Serial.print("🔥 MQ-2   (Metan/LPG):     ");
  Serial.println(q.mq2 == 1 ? "✅ TOZA" : "🚨 GAZ ANIQLANDI!");

  Serial.print("💨 MQ-7   (CO):            ");
  Serial.println(q.mq7 == 1 ? "✅ TOZA" : "🚨 CO ANIQLANDI!");

  Serial.print("🌡️  Harorat:               ");
  if (!isnan(q.harorat)) { Serial.print(q.harorat, 1); Serial.println(" °C"); }
  else Serial.println("-- (xato)");

  Serial.print("💧 Namlik:                 ");
  if (!isnan(q.namlik))  { Serial.print(q.namlik, 1);  Serial.println(" %");  }
  else Serial.println("-- (xato)");

  Serial.print("📈 Jami yuborildi: ");
  Serial.println(yuborish_soni);
  Serial.println("─────────────────────────────────────");
}

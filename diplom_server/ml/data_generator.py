...
Uzbekiston iqlimi va havo sifati patternlari asosida:
...
def soat_aqi_bazasi(soat: int) -> float:
    ...
    Uzbekiston shahri kunlik transport patterti asosida.
    """
    if 6 <= soat < 10:
...
def bosim_hisoblash(soat: int, kun_raqami: int) -> float:
    """
    Uzbekiston uchun atmosfera bosimi modeli (hPa).
...
def csv_saqlash(df: pd.DataFrame) -> None:
    ...
    print(f"CSV: {CSV_YOLI}  ({len(df):,} yozuv)")
...
def sqlite_saqlash(df: pd.DataFrame) -> None:
    ...
    if mavjud >= 1000:
        print(f"SQLite: {mavjud:,} yozuv bor, qo'shilmadi.")
        conn.close()
        return
    ...
    conn.close()
    print(f"SQLite: {len(df):,} yozuv qo'shildi (jami: {yangi_jami:,})")
...
if __name__ == "__main__":
    ...
    print(f"\nTayyor. Keyingi: python ml/train_model.py")

...
_AQI_TAVSIYALAR = [
    (0,   50,  "Havo a'lo, tashqarida faol bo'lish mumkin."),
    (51,  100, "Havo qoniqarli. Sezgir odamlar uzoq mashqlarni cheklashi kerak."),
    (101, 150, "Bolalar, keksalar, nafas kasalligi bor kishilar tashqarini qisqartirsin."),
    (151, 200, "Barcha tashqi faoliyatni kamaytirsin. Sezgir guruhlar ichkarida qolsin."),
    (201, 300, "Favqulodda holat. Niqob kiyish va ichkarida qolish tavsiya etiladi."),
    (301, 500, "O'ta jiddiy xavf. Tashqarida bo'lmang, tibbiy yordam oling."),
]
...
def get_recommendation(aqi: int) -> str:
    for lo, hi, tavsiya in _AQI_TAVSIYALAR:
        if lo <= aqi <= hi:
            return tavsiya
    return "Havo sifati o'ta xavfli. Zudlik bilan xavfsiz joyga o'ting."
...
class HavoBashorati:
    ...
    def load_model(self) -> bool:
        try:
            ...
            log.info(
                "LSTM yuklandi | MAE=%.2f RMSE=%.2f R2=%.4f",
                m.get("MAE", 0), m.get("RMSE", 0), m.get("R2", 0),
            )
            return True

        except FileNotFoundError:
            log.warning("Model topilmadi: %s", MODEL_YOLI)
